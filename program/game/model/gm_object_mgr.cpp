...
/// <summary>
/// Assumed use case :
/// Initialize function do only once when switched scenes (init() func.).
/// * to need registrate Coordinate class [or Inheritance class])
/// </summary>
void ObjectMgr::init() {
...
/// <summary>
/// Assumed use case :
/// Update function do every flames at scene update() func..
/// * to need registrate Coordinate class [or Inheritance class])
/// </summary>
/// <param name="delta_time"> seconds between frames </param>
void ObjectMgr::update(float delta_time) {
	for (int i = 0; i < hierarchy_v_.size(); i++) {
...
/// <summary>
/// Assumed use case :
/// Render parts class (create from mesh class) for every flames at scene render() func.
/// </summary>
/// <param name="camera"></param>
void ObjectMgr::render(dxe::Camera* camera) {
	for (int i = 0; i < hierarchy_v_.size(); i++) {
		for (int c = 0; c < hierarchy_v_[i].size(); c++) {
			for (int r = 0; r < hierarchy_v_[i][c].size(); r++) {
				if (hierarchy_v_[i][c][r] == nullptr) { continue; }
...
void ObjectMgr::registrateOrigine(Coordinate* origine, co_type type) {
	hierarchy_v_[static_cast<int>(type)][0].push_back(origine);
}

bool ObjectMgr::registrateCoordinate(int parent_id, std::string parent_name, Coordinate* coord, 
...
bool ObjectMgr::registrateIKCoordinate(const std::vector<coord_id_name_ik_st_>* c_ik_v) {
...
void ObjectMgr::viewCoordinateState(co_type type, view_param param) {
	int count = 0;
	int col = 10;
	int row = 5;
	int col_quantity = 20;
	...
	case ObjectMgr::view_param::pos:
		...
		break;
	case ObjectMgr::view_param::rot_deg:
...
Coordinate* ObjectMgr::getRegistratedCoordinate(int id, std::string name, co_type type) {
...
void ObjectMgr::getCoordinateDataCSV(std::string csv_path) {
	std::string str_buf;
	...
	for (int i = 3; i < 103; i++) {		// ignore headers
		...
		obj->setCoordinate(id, name,
			pos, vdr[0], vdr[1], vdr[2], vdr[3], vdr[4], vdr[5],
			tnl::Quaternion::RotationAxis(rotAxis, tnl::ToRadian(deg))
			);
		if (is_show_coordinate) { obj->setViewCoorinate(size, length); }
		// registrate coordinates
		if (parent_id == -1) {
			registrateOrigine(obj, ObjectMgr::co_type::normal);
		}
		else {
			registrateCoordinate(parent_id, parent_name, obj, co_type::normal);
		}
	}
}

bool ObjectMgr::attachIK_st(const coord_id_name_ik_st_* c_ik_v) {
